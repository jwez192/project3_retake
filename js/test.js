
    let characters = [
      {
        "id":0,
        "name":"Thorn",
        "background": "thorn_background",
        "equipment":
        {
          "head":"thorn_helm",
          "shoulders":"thorn_shoulders",
          "chest":"thorn_chest",
          "pants":"thorn_pants",
          "boots":"thorn_boots",
          "cloak":"thorn_cloak",
          "belt":"thorn_belt",
          "gloves":"thorn_gloves",
          "wrists":"thorn_wrists",
          "tabard":"thorn_tabard",
          "shirt":"thorn_shirt",
          "neck":"thorn_neck",
          "ringOne":"thorn_ring_one",
          "ringTwo":"thorn_ring_two",
          "trinketOne":"thorn_trinket_one",
          "trinketTwo":"thorn_trinket_two"
        },
        "equipmentDescription":
        {
          "head":"Sinful Aspirant's Chain Helm",
          "shoulders":"Sinful Gladiator's Chain Monnion",
          "chest":"Master Huntsman's Bandolier",
          "pants":"Sinful Gladiator's Chain Breeches",
          "boots":"Jingling Stone Stompers",
          "cloak":"Cloak of the Windspeaker",
          "belt":"Sinful Aspirant's Chain Belt",
          "gloves":"Bone Crushing Vicegrips",
          "wrists":"Fleshscale Wristwraps",
          "tabard":"Tabard of the Unshackled",
          "shirt":"Empty(shirt)",
          "neck":"Sinful Gladiator's Necklace",
          "ringOne":"Sinful Gladiator's Ring",
          "ringTwo":"Sinful Aspirant's Signet",
          "trinketOne":"Sinful Gladiator's Emblem",
          "trinketTwo":"Sinful Aspirant's Medallion"
        },
        "stats":
        {
          "health":30820,
          "maelstrom":100,
          "intellect":1397,
          "stamina":1541,
          "crit":9,
          "haste":11,
          "mastery":25,
          "vers":21
        },
        "achievements":
        [
          {
          "character":
            [
              {
              "id":0,
              "title":"epic",
              "description":"Equip an item of at least Epic quality, with a minimum item level of 183, in every slot.",
              "completed":"yes"
              },
              {
              "id":1,
              "title":"Level 60",
              "description":"Reach level 60.",
              "completed":"yes"
              },
              {
              "id":2,
              "title":"Got My Mind On My Money",
              "description":"Loot 100,000 gold.",
              "completed":"yes"
              },
              {
              "id":3,
              "title":"Safe Deposit",
              "description":"Buy 7 additional bank slots.",
              "completed":"yes"
              },
              {
              "id":4,
              "title":"Breaking the Sound Barrier",
              "description":"Learn the Master Riding skill.",
              "completed":"yes"
              }
            ]
          },
          {
              "pvp":
              [
                {
                    "id":0 ,
                    "title":"Overthrow the Coucil" ,
                    "description":"Kill representatives Moira Thaurissan, Muradin Bronzebeard and Falstad Wildhammer" , 
                    "completed":"yes"
                },
                {
                    "id":1 ,
                    "title":"50000 Honorable Kills" ,
                    "description":" Get 50000 honorable kills" , 
                    "completed":"yes"
                },
                {
                    "id":2 ,
                    "title":"Honorable Medallion" ,
                    "description":"Enter a Battleground or Arena and learn the spell Honorable Medallion" , 
                    "completed":"yes"
                },
                {
                    "id":3 ,
                    "title":"Call to Arms!" ,
                    "description":"Complete 100 battlegrounds at max level" , 
                    "completed":"yes"
                },
                {
                    "id":4 ,
                    "title":"Duel-icious" ,
                    "description":"Win a duel against another player" , 
                    "completed":"yes"
                }
              ]
          },
          {
              "quests":
              [
                {
                    "id":0,
                    "title":"1000 Daily Quests Completed",
                    "description":"Complete 1000 Daily Quests",
                    "completed":"yes"               
                },
                {
                    "id":1,
                    "title":"1000 World Quests Completed",
                    "description":"Complete 1000 World Quests",
                    "completed":"yes"
                },
                {
                    "id":2,
                    "title":"Exiles Reach",
                    "description":"Escape the isle of Exiles Reach and embark upon a greater adventure.",
                    "completed":"yes"
                },
                {
                    "id":3,
                    "title":"3000 Quests Completed",
                    "description":"Complete 3000 quests",
                    "completed":"yes"
                },
                {
                    "id":4,
                    "title":"Justly Rewarded",
                    "description":"Make 30,0000 gold from quests.",
                    "completed":"yes"
                }
              ]
          },
          {
              "exploration":
              [
                  {
                      "id":0,
                      "title":"Battle For Azeroth Explorer",
                      "description":"Explort the regions of Kul Tiras and Zandalar",
                      "completed":"yes"
                  },
                  {
                      "id":1,
                      "title": "Broken Isle Explorer",
                      "description": "Explort the regions of the Broken Isles",
                      "completed":"yes"
                  },
                  {
                      "id":2,
                      "title": "Going Down!",
                      "description":"Fall 65 yards without dying.",
                      "completed":"yes"
                  },
                  {
                      "id":3,
                      "title":"Eastern Kingdoms Explorer",
                      "description":"Explore the regions of Eastern Kingdoms.",
                      "completed":"yes"
                  },
                  {
                      "id":4,
                      "title":"Kalimdor Explorer",
                      "description":"Explore the regions of Kalimdor.",
                      "completed":"yes"
                  }
              ]
          }

        ]
      },
      {
        "id":1,
        "name":"Icemechanic",
        "background":"icemechanic_background",
        "equipment":
        {
          "head":"icemechanic_helm",
          "shoulders":"icemechanic_shoulders",
          "chest":"icemechanic_chest",
          "pants":"icemechanic_pants",
          "boots":"icemechanic_boots",
          "cloak":"icemechanic_cloak",
          "belt":"icemechanic_belt",
          "gloves":"icemechanic_gloves",
          "wrists":"icemechanic_wrists",
          "tabard":"icemechanic_tabard",
          "shirt":"icemechanic_shirt",
          "neck":"icemechanic_neck",
          "ringOne":"icemechanic_ring_one",
          "ringTwo":"icemechanic_ring_two",
          "trinketOne":"icemechanic_trinket_one",
          "trinketTwo":"icemechanic_trinket_two"
        },
        "equipmentDescription":
        {
          "head":"Shattered Fragments of Sindragosa",
          "shoulders":"Soul-Siphon Mantle",
          "chest":"Runebound Tunic",
          "pants":"Runebound Leggings",
          "boots":"Nefarious Light-Step Slippers",
          "cloak":"Drape of the Spirited Hunt",
          "belt":"Enhanced Worldscorcher Cinch",
          "gloves":"Manawracker Gloves",
          "wrists":"Man'ari Pyromancer Cuffs",
          "tabard":"Renowned Guild Tabard",
          "shirt":"Sightless Mantle",
          "neck":"Collar of Null-Flame",
          "ringOne":"Seal of the Portalmaster",
          "ringTwo":"Shard of the Exodar",
          "trinketOne":"Reality Breacher",
          "trinketTwo":"Sheath of Asara"
        },
      },
      {
        "id":2,
        "name":"Grandil",
        "background":"grandil_background",
        "equipment":
        {
          "head":"grandil_helm",
          "shoulders":"grandil_shoulders",
          "chest":"grandil_chest",
          "pants":"grandil_pants",
          "boots":"grandil_boots",
          "cloak":"grandil_cloak",
          "belt":"grandil_belt",
          "gloves":"grandil_gloves",
          "wrists":"grandil_wrists",
          "tabard":"grandil_tabard",
          "shirt":"grandil_shirt",
          "neck":"grandil_neck",
          "ringOne":"grandil_ring_one",
          "ringTwo":"grandil_ring_two",
          "trinketOne":"grandil_trinket_one",
          "trinketTwo":"grandil_trinket_two"
        },
        "equipmentDescription":
        {
          "head":"Desolate Leather Helm",
          "shoulders":"Waylight Epaulets",
          "chest":"Waylight Tunic",
          "pants":"Deceitful Agent's Breeches",
          "boots":"Desolate Leather Treads",
          "cloak":"Blighted Margrave's Cloak",
          "belt":"Deceitful Agent's Belt",
          "gloves":"Desolate Leather Gauntlets",
          "wrists":"Desolate Leather Armguards",
          "tabard":"Illustrious Guild Tabard",
          "shirt":"Formal White Shirt",
          "neck":"Reconstructed Family Locket",
          "ringOne":"Band of Chronicled Deeds",
          "ringTwo":"Reverberating Silver Band",
          "trinketOne":"Misfiring Centurion Controller",
          "trinketTwo":"Phial of Putrefaction"
        },
        "stats":{
            "health":26471,
            "mana": 5253,
            "agility":538,
            "stamina":1226,
            "crit":19,
            "haste":13,
            "mastery":8,
            "vers":4
        }
      },
      {
        "id":3,
        "name":"Ghoultak",
        "background":"ghoultak_background",
        "equipment":
        {
          "head":"ghoultak_helm",
          "shoulders":"ghoultak_shoulders",
          "chest":"ghoultak_chest",
          "pants":"ghoultak_pants",
          "boots":"ghoultak_boots",
          "cloak":"ghoultak_cloak",
          "belt":"ghoultak_belt",
          "gloves":"ghoultak_gloves",
          "wrists":"ghoultak_wrists",
          "tabard":"ghoultak_tabard",
          "shirt":"ghoultak_shirt",
          "neck":"ghoultak_neck",
          "ringOne":"ghoultak_ring_one",
          "ringTwo":"ghoultak_ring_two",
          "trinketOne":"ghoultak_trinket_one",
          "trinketTwo":"ghoultak_trinket_two"
        },
        "equipmentDescription":
        {
          "head":"Sinful Aspirant's Silk Hood",
          "shoulders":"Sinful Aspirant's Silk Mantle",
          "chest":"Bloodsilk Robe",
          "pants":"Bladesworn Conjurer's Leggings",
          "boots":"Shadowlace Footwraps",
          "cloak":"Sinful Aspirant's Drape",
          "belt":"Cinch of Infinite Tightness",
          "gloves":"Sinful Aspirant's Silk Gloves",
          "wrists":"Grim Pursuant's Maille",
          "tabard":"Empty(tabard)",
          "shirt":"Empty(shirt)",
          "neck":"Chain of the Purpose-Bound",
          "ringOne":"Entwined Gorger Tendril",
          "ringTwo":"Sinful Aspirant's Ring",
          "trinketOne":"Sinful Aspirant's Insignia of Alacrity",
          "trinketTwo":"Sinful Aspirant's Medallion"
        },
        "stats":{
            "health":25940,
            "mana": 50000,
            "intellect":1088,
            "stamina":1297,
            "crit":16,
            "haste":8,
            "mastery":18,
            "vers":13
        }
      },
      {
        "id":3,
        "name":"Vinha",
        "background":"vinha_background",
        "equipment":
        {
          "head":"vinha_helm",
          "shoulders":"vinha_shoulders",
          "chest":"vinha_chest",
          "pants":"vinha_pants",
          "boots":"vinha_boots",
          "cloak":"vinha_cloak",
          "belt":"vinha_belt",
          "gloves":"vinha_gloves",
          "wrists":"vinha_wrists",
          "tabard":"vinha_tabard",
          "shirt":"vinha_shirt",
          "neck":"vinha_neck",
          "ringOne":"vinha_ring_one",
          "ringTwo":"vinha_ring_two",
          "trinketOne":"vinha_trinket_one",
          "trinketTwo":"vinha_trinket_two"
        },
        "equipmentDescription":
        {
          "head":"Mistdancer Helm",
          "shoulders":"Exceptional Stormshroud Shoulders",
          "chest":"Stained Shadowcraft Tunic",
          "pants":"Jambani Breeches",
          "boots":"Mistdancer Footpads",
          "cloak":"Inherited Cape of the Black Baron",
          "belt":"Jambani Waistwrap",
          "gloves":"Mistdancer Handguards",
          "wrists":"Mistdancer Cuffs",
          "tabard":"vinha_tabard",
          "shirt":"vinha_shirt",
          "neck":"Heart of Azeroth",
          "ringOne":"Mistdancer Band of Onslaught",
          "ringTwo":"Mistdancer Ring of Onslaught",
          "trinketOne":"Mistdancer Idol of Rage",
          "trinketTwo":"Mistdancer Stone of Rage"
        },
        "stats":{
            "health":25940,
            "mana": 50000,
            "intellect":1088,
            "stamina":1297,
            "crit":16,
            "haste":8,
            "mastery":18,
            "vers":13
        }
      }
    ]
  